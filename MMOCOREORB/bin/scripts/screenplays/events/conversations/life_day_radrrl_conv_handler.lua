...
function lifeDayRadrrlConvoHandler:getInitialScreen(pPlayer, pNpc, pConvTemplate)
	...
	if readScreenPlayData(pPlayer, readStringSharedMemory("lifeDayScreenplayName"), "completeagain") == "1" or readData(playerID .. ":lifeDayRadrrl1") == 1 then
...
function lifeDayRadrrlConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	if screenID == "always_remember" or screenID == "participate" then
		writeData(playerID .. ":lifeDayRadrrl1", 1)
		if readData(playerID .. ":lifeDayAnarra1") == 1 and readData(playerID .. ":lifeDayTebeurra1") == 1 then
